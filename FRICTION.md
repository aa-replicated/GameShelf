...
6.  **Invalid config during setup leaves install in an unrecoverable state** — If a config value fails validation (e.g. a password that's too short), the install fails and there is no way to retry or correct the value. The only option is to start over with a fresh install. There is no "go back and fix config" path in the EC3 setup UI.
